...
class ValidateSubmissionHandler(logger: Logger, private val submissionService: SubmissionService) : AbstractValidationHandler<Command2Type>(logger) {
    ...
    private fun ValidateSubmissionRequest.validateTextAttributes(): Result<ValidateSubmissionRequest, DataErrors.Validation.EmptyString> {
        try {
            id.checkForBlank("submission.id")
            candidates.forEachIndexed { i, candidate ->
                ...
                documents?.forEachIndexed {k, document ->
